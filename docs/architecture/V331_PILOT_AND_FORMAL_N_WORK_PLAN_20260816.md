...
| A7 | 发布候选代码 | 只允许目标分支；本地零API验证通过 | 远端candidate SHA | 进行中 |
| G1 | Windows准入 | 准确SHA完整pytest通过；plan-only核对24/576/4800/8 | 用户测试记录、clean SHA | 未开始 |
| G2 | 费用确认 | 用户确认24-block行政费用容忍度；不能用调用数伪称精确费用 | 授权记录修订 | 未开始 |
| B1 | 执行Pilot | P001–P024全部valid；不换seed；中断按合同恢复 | seed/attempt/validity/estimand ledgers | 未开始 |
| B2 | 离线方差分析 | 24个独立blocks；Pilot mean/sign不进入设计 | components、planning SD、manifest | 未开始 |
| B3 | 离线OC与N_required | ≥200,000 replications/场景；FWER数值门禁；四场景全通过 | OC curve、selection、summary | 未开始 |
...
FORMAL BLOCKS: 0
FORMAL INFERENCE: 0
NEXT GATE: PUBLISH CANDIDATE SHA -> WINDOWS TEST -> COST RECONFIRMATION
```
