...
正式 F001-F010 的统计推断已经冻结并归档，本模块不会计算新的 formal
p-value、Holm correction 或重新定义 estimand。
"""
...
def analyze_run(run_dir: Path) -> dict:
    """生成便于快速检查的 condition-level 描述统计。"""
    ...
    # 认知层：T30 信任和 post-crisis T6-T30 平均信任。
    summaries = []
    ...
    # 行为层：只在实际 category-purchase opportunities 中计算 share。
    demand_summary = []
            ...
            demand_summary.append(
                {
                    ...
                    "realized_choice_share_t6_t30": _mean(
                        1.0 if str(r["focal_brand_chosen"]).lower() == "true" else 0.0
                        for r in post
                    ),
                }
            )
    ...
    payload = {
        "schema_version": "task005_fmcg_v32_clean_analysis1.1",
        "status": "PASS",
