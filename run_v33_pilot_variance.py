...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "the 24 pre-registered Real-LLM Pilot blocks."
        ),
    )
    ...
    mode.add_argument("--resume-real-pilot", type=Path, metavar="SUITE_DIR")
    ...
    if args.plan_only:
        ...
        if any(supplied) and not all(supplied):
            parser.error(
                "plan-only accepts --provider-call-ceiling and "
    ...
    payload = run_sync(
        ...
        resume_suite=args.resume_real_pilot,
    )
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if payload["status"] == "PASS" else 2
