"""TASK_005 real-LLM manipulation-check pilot v3 runner.

Pilot v3 freezes the converged three-dimensional construct manipulation, strict raw semantic
...
PILOT_ID = "task005-real-manipulation-pilot-v3"
MASTER_SEED = 2026080904
...
REAL_MODE_REJECTION = "PILOT_V3_REAL_EXECUTION_NOT_AUTHORIZED"
ACTIVATION_TOKEN = "TASK005_REAL_MANIPULATION_V3_ACTIVATE_20260809_01"
...
ACTIVATION_ARTIFACT = (
    ...
    / "real_llm_manipulation_pilot_v3_activation1.0.json"
)
SOURCE_FREEZE_FILES = (
    ...
    ".kiro/specs/task005-replication-inference/manipulation_construct_validity_amendment3.0.json",
    ".kiro/specs/task005-replication-inference/mechanism_auditability_schema_amendment1.1.json",
    ".kiro/specs/task005-replication-inference/real_llm_manipulation_check_pilot_v3_contract1.0.json",
)
...
class PilotSemanticValidationError(PilotContractError):
    """Raised when a raw real semantic response violates the v3 schema."""
        ...
        proc = subprocess.run(
            [
                ...
                (
                    ...
                    "($_.CommandLine -match 'task005-real-manipulation-pilot-v3|run_experiments.py') "
                    "} | "
...
def build_pilot_seed_ledger() -> list[dict]:
    ...
    if tuple(row["replicate_id"] for row in rows) != ALLOWED_REPLICATE_IDS:
        raise PilotContractError("pilot-v3 replicate IDs are not exactly R001-R002")
    return rows
        ...
        pairs.append(
            {
                ...
                "D_arousal_descriptive": _float(e["semantic_arousal"]) - _float(r["semantic_arousal"]),
...
def _summarize_pooled_pairs(pairs: list[Mapping]) -> dict:
    summary = {
        ...
        "P_VALUES_COMPUTED": False,
        "PILOT_ONLY": True,
        "semantic_arousal_role": "descriptive_only",
...
async def _execute_replicate_block(
    ...
    *,
    ...
    try:
        ...
        write_seed_ledger_subset_csv(
            build_pilot_seed_ledger(),
            [replicate_id],
            block_dir / "seed_ledger.csv",
        )
