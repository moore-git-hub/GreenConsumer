"""Scenario-v3.2 semantic appraisal with an explicit peer-approval construct."""
...
class GreenCognitionV32Plugin(ReflectPlugin):
    """Parse message semantics without deciding purchase or posting behavior."""
    ...
    def _parse(response):
        ...
        if isinstance(response, list):
            ...
            return GreenCognitionV32Plugin._parse(response[0])
    ...
    async def execute(self, current_tick: int) -> None:
        ...
        global_news = [row for row in observations if row.get("source") == "Global News"]
        clarification = [
            row for row in observations if row.get("source") == "Enterprise_Clarification"
        ]
        social = [
            row
        ...
        if social:
            for row in social[:2]:
                parts.append(f"[Social Feed] {str(row.get('content', ''))[:180]}")
        ...
        try:
            ...
            state_plugin.add_to_memory(
                ...
                (
                    ...
                    f"V={semantic['valence']:+.2f} C={semantic['credibility']:.2f} "
            ...
            print(
                f"[Reflect-v3.2 ERROR] {agent.agent_id}: "
