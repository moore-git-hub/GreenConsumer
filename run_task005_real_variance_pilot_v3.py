"""TASK_005 variance pilot v3 audited real-execution runner.

Offline acceptance runs the real run_experiments._run_with_patch orchestration
with a deterministic fake inner router. Real variance-v3 execution is available
...
MASTER_SEED = 2026081003
REPLICATION_BLOCKS = 10
ALLOWED_REPLICATE_IDS = tuple(f"R{i:03d}" for i in range(1, 11))
REAL_MODE_REJECTION = "VARIANCE_V3_REAL_EXECUTION_NOT_AUTHORIZED"
ACTIVATION_TOKEN = "TASK005_REAL_VARIANCE_V3_ACTIVATE_20260810_01"
EXPECTED_BRANCH = "redesign/task005-mechanism-v2"
EXPECTED_START_HEAD = "a6b957c55c34ada19477259602c3d4a62eeab1f4"
MODEL = "qwen-plus"
TEMPERATURE = 0.3
SPEC_DIR = Path(".kiro/specs/task005-replication-inference")
ACTIVATION_ARTIFACT = SPEC_DIR / "real_llm_variance_pilot_v3_activation1.0.json"
RESULT_ARTIFACT = SPEC_DIR / "real_llm_variance_pilot_v3_result1.0.json"
...
SOURCE_FREEZE_FILES = (
    ...
    ".kiro/specs/task005-replication-inference/real_llm_variance_pilot_v3_contract1.0.json",
    ...
    ".kiro/specs/task005-replication-inference/real_llm_variance_pilot_v2_disposition1.0.json",
    ".kiro/specs/task005-replication-inference/async_model_router_forensic1.0.json",
)


class VarianceV3Error(RuntimeError):
    """Raised when the v3 audited variance contract is violated."""
...
def build_pilot_seed_ledger() -> list[dict]:
    ...
    if tuple(row["replicate_id"] for row in rows) != ALLOWED_REPLICATE_IDS:
        raise VarianceV3Error("replicate IDs must be R001-R010")
    return rows
...
def select_variance_conditions():
    ...
    if tuple(cfg.exp_id for cfg in configs) != tuple(EXECUTION_ORDER):
        raise VarianceV3Error("condition order mismatch")
    return configs
...
def select_variance_conditions_for_block(ledger_row: Mapping):
    ...
    if len(configs) != v1.CONDITIONS_PER_BLOCK:
        raise VarianceV3Error("block must contain exactly 9 conditions")
    if tuple(cfg.exp_id for cfg in configs) != tuple(EXECUTION_ORDER):
        raise VarianceV3Error("block condition order mismatch")
    if {cfg.random_seed for cfg in configs} != {simulation_seed}:
        raise VarianceV3Error("block condition seeds must equal ledger simulation_seed")
    return configs
...
def current_source_hashes() -> dict[str, str]:
    ...
    for rel in SOURCE_FREEZE_FILES:
        path = Path(rel)
        if not path.exists():
            raise VarianceV3Error(f"source freeze file missing: {rel}")
...
def _git_stdout(args: list[str]) -> str:
    ...
    if proc.returncode != 0:
        raise VarianceV3Error(f"git {' '.join(args)} failed")
...
def _assert_tracked_clean() -> None:
    if _git_stdout(["diff", "--name-only"]):
        raise VarianceV3Error("tracked working tree must be clean")
    if _git_stdout(["diff", "--cached", "--name-only"]):
        raise VarianceV3Error("staged changes must be clean")


def _load_json(path: Path) -> dict:
    if not path.exists():
        raise VarianceV3Error(f"required artifact missing: {path.as_posix()}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise VarianceV3Error(f"{path.name} must be a JSON object")
    return payload
...
def _assert_git_activation_state(payload: Mapping) -> None:
    ...
    if activation_head:
        if head != activation_head:
            raise VarianceV3Error("activation HEAD mismatch")
        parent = _git_stdout(["rev-parse", f"{head}^"])
        if parent != activation_code_head:
            raise VarianceV3Error("activation parent HEAD mismatch")
    elif activation_code_head:
        parent = _git_stdout(["rev-parse", f"{head}^"])
        if parent != activation_code_head:
            raise VarianceV3Error("activation code parent mismatch")
    else:
        raise VarianceV3Error("activation_code_head missing")
...
def _validate_model_config() -> None:
    ...
    if "model: qwen-plus" not in text:
        raise VarianceV3Error("model config must use qwen-plus")
    if "temperature: 0.3" not in text:
        raise VarianceV3Error("model config temperature must be 0.3")
    if 'api_key: "__FROM_ENV_DASHSCOPE_API_KEY__"' not in text:
        raise VarianceV3Error("model config must use DASHSCOPE env placeholder")
...
def _validate_clean_output_state(output_dir: Path) -> None:
    ...
    if any(value != 0 for value in counts.values()):
        raise VarianceV3Error("real variance-v3 attempt markers already exist")
...
def _validate_real_execution_authorized(
    ...
    *,
    ...
    if not _credential_available():
        raise VarianceV3Error("DASHSCOPE_API_KEY credential unavailable")
    if require_clean_output:
        _validate_clean_output_state(output_dir)
    return payload


async def _build_and_close_router_zero_call(ledger_row: Mapping, builder) -> dict:
    router = None
    try:
        asyncio.get_running_loop()
        router = builder(ledger_row)
        await _close_router_resource(router)
        return {
            "REAL_ROUTER_BUILD": "PASS",
            "REAL_ROUTER_CLOSE": "PASS",
            "REAL_LLM_CALLS": 0,
            "AUDIT_LOG_ROWS": 0,
            "MODEL_CALLS": 0,
        }
        ...
        return {
            "REAL_ROUTER_BUILD": "FAIL" if router is None else "PASS",
            "REAL_ROUTER_CLOSE": "NOT_EVALUATED" if router is None else "FAIL",
            "REAL_LLM_CALLS": 0,
            "AUDIT_LOG_ROWS": 0,
            "MODEL_CALLS": 0,
...
def real_router_zero_call_preflight(
    output_dir: Path,
    activation_token: str | None,
    *,
    offline_tripwire: bool = False,
) -> dict:
    if offline_tripwire:
        old_value = os.environ.get("DASHSCOPE_API_KEY")
        os.environ["DASHSCOPE_API_KEY"] = "sanitized-zero-call-preflight"
    else:
        _validate_real_execution_authorized(
            output_dir,
            activation_token,
    ...
    try:
        ledger_row = build_pilot_seed_ledger()[0]
        result = asyncio.run(_build_and_close_router_zero_call(ledger_row, _build_real_inner_router))
    finally:
        if offline_tripwire:
            import socket

            assert old_create is not None and old_socket is not None
    ...
    result.update({
        ...
        "master_seed": MASTER_SEED,
        "replicate_id": "R001",
        "requested_llm_seed": int(build_pilot_seed_ledger()[0]["requested_llm_seed"]),
        "credential_available": True if offline_tripwire else _credential_available(),
        "credential_value_recorded": False,
        "EXTERNAL_PROVIDER_REQUESTS": external_calls,
        "EXTERNAL_NETWORK_CALLS": external_calls,
        "attempt_counts": _output_attempt_counts(output_dir),
        "BLOCK_ATTEMPTS_CREATED": any(_output_attempt_counts(output_dir).values()),
    })
    return result
...
def _network_hash_from_result(result: Mapping) -> str:
    ...
    if not nodes or not edges:
        raise VarianceV3Error(f"{result.get('exp_id')} missing production network audit rows")
    payload = {
...
def _profile_hash_from_result(result: Mapping) -> str:
    ...
    if len(tick1) != v1.AGENT_COUNT:
        raise VarianceV3Error(f"{result.get('exp_id')} missing initialized profile audit rows")
...
def _identity_json_hash(path: Path, identity_key: str) -> str:
    ...
    if not isinstance(rows, list) or len(rows) != v1.CONDITIONS_PER_BLOCK:
        raise VarianceV3Error(f"{path.name} must contain 9 condition rows")
    hashes = {row.get(identity_key) for row in rows}
    if len(hashes) != 1:
        raise VarianceV3Error(f"{path.name} condition hashes are not identical")
...
def _mechanism_condition_rows(rows: list[Mapping], exp_id: str) -> list[Mapping]:
    ...
    if len(selected) != v1.AGENT_COUNT * v1.TOTAL_TICKS:
        raise VarianceV3Error(f"{exp_id} mechanism rows must be 600")
    return selected
...
def _assert_production_pretreatment_alignment(mechanism_rows: list[Mapping]) -> str:
    ...
    for cfg in configs:
        ...
        if len(c_by_agent) != v1.AGENT_COUNT or len(s_by_agent) != v1.AGENT_COUNT:
            raise VarianceV3Error(f"{cfg.exp_id} pretreatment tick {tick} must contain 20 agents")
        if set(c_by_agent) != set(s_by_agent):
            raise VarianceV3Error(f"{cfg.exp_id} pretreatment agent set mismatch")
        for agent_id in sorted(c_by_agent):
            for field in fields:
                if str(c_by_agent[agent_id].get(field)) != str(s_by_agent[agent_id].get(field)):
                    raise VarianceV3Error(f"{cfg.exp_id} pretreatment mismatch {field} {agent_id} T{tick}")
    return "PASS"
...
def _as_bool(value) -> bool:
    ...
    if isinstance(value, str) and value in {"True", "False"}:
        return value == "True"
    raise VarianceV3Error(f"invalid audit boolean {value!r}")


def _validate_audit_rows(call_rows: list[Mapping], replicate_id: str, ledger_row: Mapping) -> dict:
    if not call_rows:
        raise VarianceV3Error("AUDIT_LOG_ROWS must be > 0")
    condition_set = set(EXECUTION_ORDER)
    seen = set()
    categories: dict[str, int] = {}
    for index, row in enumerate(call_rows, start=1):
        if row.get("pilot_id") != PILOT_ID:
            raise VarianceV3Error("audit row pilot_id mismatch")
        if row.get("replicate_id") != replicate_id:
            raise VarianceV3Error("audit row replicate_id mismatch")
        if int(row.get("call_index", 0)) != index or index in seen:
            raise VarianceV3Error("audit call_index must be contiguous 1..N")
        seen.add(index)
        if row.get("condition") not in condition_set:
            raise VarianceV3Error("audit row condition outside matrix")
        tick = int(row.get("tick", 0))
        if tick < 1 or tick > v1.TOTAL_TICKS:
            raise VarianceV3Error("audit row tick out of range")
        for field in ("prompt_sha256", "response_sha256"):
            value = str(row.get(field, ""))
            if len(value) != 64 or any(ch not in "0123456789abcdef" for ch in value.lower()):
                raise VarianceV3Error(f"audit {field} must be 64 hex")
        if row.get("model") != MODEL:
            raise VarianceV3Error("audit model mismatch")
        if abs(float(row.get("temperature")) - TEMPERATURE) > 1e-12:
            raise VarianceV3Error("audit temperature mismatch")
        if int(row.get("requested_llm_seed")) != int(ledger_row["requested_llm_seed"]):
            raise VarianceV3Error("audit requested_llm_seed mismatch")
        category = str(row.get("prompt_category"))
        categories[category] = categories.get(category, 0) + 1
        if category not in ALLOWED_PROMPT_CATEGORIES:
            raise VarianceV3Error("audit prompt category is not allowed")
        if category == "semantic":
            if not _as_bool(row.get("response_parse_ok")):
                raise VarianceV3Error("semantic audit response_parse_ok must be true")
            if not _as_bool(row.get("semantic_schema_ok")):
                raise VarianceV3Error("semantic audit semantic_schema_ok must be true")
            if str(row.get("validation_error_type", "")):
                raise VarianceV3Error("semantic audit validation_error_type must be empty")
            if str(row.get("error_type", "")):
                raise VarianceV3Error("semantic audit error_type must be empty")
    return categories
...
def validate_block_gates(block_dir: Path, replicate_id: str, ledger_row: Mapping | None = None) -> dict:
    ...
    if len(manifest) != v1.CONDITIONS_PER_BLOCK:
        raise VarianceV3Error("condition_manifest must contain 9 rows")
    if tuple(row.get("exp_id") for row in manifest) != tuple(EXECUTION_ORDER):
        raise VarianceV3Error("condition_manifest order mismatch")
    if manifest_seed_set != {int(ledger_row["simulation_seed"])}:
        raise VarianceV3Error("condition_manifest random_seed must equal ledger simulation_seed")
    ...
    if summary_path.exists():
        ...
        if summary.get("status") == "PASS" and int(summary.get("MODEL_CALLS", -1)) != len(call_rows):
            raise VarianceV3Error("MODEL_CALLS must equal AUDIT_LOG_ROWS")
    ...
    if len(mechanism_keys) != v1.CONDITIONS_PER_BLOCK * v1.AGENT_COUNT * v1.TOTAL_TICKS:
        raise VarianceV3Error("mechanism_records row count must be 5400")
    if len(set(mechanism_keys)) != len(mechanism_keys):
        raise VarianceV3Error("duplicate mechanism_records join key")
    ...
    if len(control_agents) != v1.AGENT_COUNT:
        raise VarianceV3Error("control mechanism agent set must contain 20 agents")
    for cfg in strategy_configs:
        if _agent_set_for_condition(mechanism_rows, cfg.exp_id) != control_agents:
            raise VarianceV3Error(f"{cfg.exp_id} mechanism agent set mismatch")
...
def run_offline_synthetic_estimator_fixture(output_dir: Path, *, blocks: int = 2) -> dict:
    ...
    if blocks != 2:
        raise VarianceV3Error("synthetic estimator fixture is frozen at 2 blocks")
...
def _build_real_inner_router(ledger_row: Mapping):
    ...
    if not api_key or api_key.startswith("__") or api_key.upper() in {"REDACTED", "PLACEHOLDER"}:
        raise VarianceV3Error("DASHSCOPE_API_KEY must be set for real variance-v3 execution")
...
async def execute_variance_block(
    ...
    *,
    ...
    if execution_mode not in {"offline-fake", "real"}:
        raise VarianceV3Error("execution_mode must be offline-fake or real")
    ...
    try:
        ...
        if router_builder is None:
            router_builder = _build_real_inner_router
        inner_router = router_builder(ledger_row)
        ...
        for cfg in configs:
            ...
            if cfg.is_control:
                ...
                if router.miss_count != 0:
                    raise VarianceV3Error(f"{cfg.exp_id} replay miss count must be zero")
        ...
        summary = {
            ...
            "ASYNC_ROUTER_BUILD_CONTEXT": "PASS",
...
def _run_variance_blocks(output_dir: Path, ledger_rows: list[Mapping], *, execution_mode: str, router_builder) -> dict:
    output_dir.mkdir(parents=True, exist_ok=True)
    block_summaries = []
    ...
    return {
        ...
        "ASYNC_ROUTER_BUILD_CONTEXT": "PASS" if all(
            row.get("ASYNC_ROUTER_BUILD_CONTEXT") == "PASS"
            for row in block_summaries
            if row.get("status") == "PASS"
        ) else "NOT_EVALUATED",
...
def run_production_path_fake_acceptance(output_dir: Path, *, blocks: int = 2) -> dict:
    if blocks != 2:
        raise VarianceV3Error("production-path fake acceptance is frozen at 2 blocks")
    ledger = build_pilot_seed_ledger()
    return _run_variance_blocks(
        output_dir,
        ledger[:blocks],
        execution_mode="offline-fake",
        router_builder=lambda _row: DeterministicFakeInnerRouter(),
    )
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--real-router-zero-call-preflight", action="store_true")
    parser.add_argument("--offline-tripwire", action="store_true")
    ...
    try:
        ...
        if args.offline_test:
            ...
            return 0
        if args.real_router_zero_call_preflight:
            out = Path(args.output_dir) if args.output_dir else PILOT_OUTPUT_ROOT
            print(json.dumps(
                real_router_zero_call_preflight(
                    out,
                    args.activation_token,
                    offline_tripwire=args.offline_tripwire,
                ),
                sort_keys=True,
            ))
            return 0
        ...
        print(f"ERROR: {label}")
        return 2 if isinstance(exc, VarianceV3Error) or type(exc).__name__ == "VarianceV3Error" else 1
