...
| Semantic audit | Explicit one-sentence appraisal reason | `greenconsumer_v32/runner.py::_agent_thought_rows`; `plugins/agent/plan/ConsumerPlanV33Plugin.py` | `agent_thoughts.csv`, `cognitive_records.csv` | Implemented; not hidden chain-of-thought. If a deterministic rule triggers posting, the same explicit reason may be reused as UGC text |
...
| Consumer UGC | Rule-triggered posting with explicit appraisal reason reused as post content when available | `plugins/agent/plan/ConsumerPlanV33Plugin.py`; `simulation_core.py` | posting events, `post_content`, social observation records | Posting is rule-governed rather than chosen by the LLM, but propagated wording is model-generated and requires a separate content-validity caveat |
...
| Behavior bridge | Cognitive-layer TPB-style purchase intention | `purchase_mechanism_v3.py`; `mechanism_v33.py` | `purchase_intention` | Implemented; simulated cognitive state, not observed consumer response and not identical to the downstream micro-buyer choice probability |
| Demand | Persona-specific renewal opportunities, micro-buyer-level intention recomputation, conditional brand choice and bounded-EWMA loyalty | `purchase_mechanism_v33.py`; `greenconsumer_v33/demand.py` | `demand_opportunities.csv`, `choice_curves.csv` | Implemented; choice probability combines current psychological state with micro-buyer PBC, preference offset and realized prior-choice loyalty. M=25 is numerical resolution, not replication |
...
- treating rule-triggered, LLM-worded UGC as observed consumer-authored content or as independently validated social discourse;
...
- equating cognitive-layer purchase intention with downstream conditional brand-choice probability or realized market share;
