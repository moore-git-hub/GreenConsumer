"""Scenario-v3.2 cognitive transition and posting plan.

Purchase is deliberately absent from this AgentKernel plan.  The selected FMCG
estimand is repeat focal-brand choice conditional on a category-purchase
occasion, evaluated by the separate audited demand layer.
"""
...
SCHEMA = "consumer-plan-3.2"


class ConsumerPlanV32Plugin(PlanPlugin):
    async def init(self):
        pass
    ...
    async def execute(self, current_tick: int) -> None:
        ...
        # Keep the v2 trust/Att/memory transition, but explicitly disable its
        # legacy valence-to-SN branch.  SN is updated below from peer approval.
        ...
        plan = {
            "mechanism_schema_version": MECHANISM_V2_SCHEMA,
            ...
            "reason": (
                f"TPB-v3.2 I={purchase_intention:.3f} "
            ...
            "decay_lambda": 1 - MEMORY_RETENTION,
            "quiet_ticks": quiet_ticks,
            ...
            "decay_rate_raw": 1 - MEMORY_RETENTION,
            "sensitivity_multiplier": 1.0,
            "trust_clipped_at_bound": final_trust <= 0 or final_trust >= 10,
            "anchor_update_branch": (
                "mechanism_v32_clarification"
                if has_clarification
                else (
                    "mechanism_v32_observation"
                    if had_observation
                    else "mechanism_v32_quiet"
                )
            ),
