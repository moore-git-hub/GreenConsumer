...
def select_public_exposure_nodes(nodes, base_seed, rate=PUBLIC_EXPOSURE_RATE):
    # Condition-blocked organic exposure to a public corporate statement.
    ...
    if r <= 0.0:
        return []
    k = max(1, int(round(len(ordered) * r)))
    k = min(k, len(ordered))
    rng = random.Random(int(base_seed) + 92317)
    return sorted(rng.sample(ordered, k))


def one_hop_amplification_nodes(graph, seed_nodes):
    # Equal-budget channel amplification generated by actual topology.
    seeds = {str(node) for node in seed_nodes}
    reached = set()
    if graph.is_directed():
        for seed in seeds:
            if seed in graph:
                reached.update(str(node) for node in graph.successors(seed))
    else:
        for seed in seeds:
            if seed in graph:
                reached.update(str(node) for node in graph.neighbors(seed))
    reached.difference_update(seeds)
    return sorted(reached)


def build_exposure_audit(graph, exp_id, public_nodes, paid_seed_nodes, amplified_nodes):
    public = {str(node) for node in public_nodes}
    paid = {str(node) for node in paid_seed_nodes}
    amplified = {str(node) for node in amplified_nodes}
    rows = []
    for node in sorted(str(node) for node in graph.nodes()):
        ...
        if node in amplified:
            modes.append("paid_one_hop")
        rows.append({
            "exp_id": exp_id,
            "agent_id": node,
            "public_organic": node in public,
            "paid_seed": node in paid,
            "paid_one_hop": node in amplified,
            "reached": bool(modes),
            "exposure_modes": ";".join(modes),
        })
    return rows
